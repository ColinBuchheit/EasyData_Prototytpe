...
export const connectPostgres = async (userId: number, dbType: string, cloudProvider: string) => {
  try {
    const credentials = await fetchCloudCredentials(userId, dbType, cloudProvider);
    if (!credentials) {
      throw new Error("❌ Missing credentials for PostgreSQL connection.");
    }

    const connectionManager = new ConnectionManager(userId, dbType, credentials);
    await connectionManager.connect(credentials);

    logger.info(`✅ PostgreSQL connection established for User ${userId}`);

    await fetchDatabaseSchema(userId, dbType);
    return connectionManager;
  } catch (error) {
    logger.error(`❌ PostgreSQL connection failed for User ${userId}: ${error}`);
    throw error;
  }
};
